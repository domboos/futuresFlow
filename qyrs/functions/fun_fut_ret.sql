CREATE OR REPLACE FUNCTION dbo.fun_fut_ret(
	contract_num integer)
    RETURNS TABLE(series_id bigint, px_date date, px_close_ret double precision, expiry date, contract_id bigint)
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION dbo.fun_fut_ret(integer)
    OWNER TO bood;

GRANT EXECUTE ON FUNCTION dbo.fun_fut_ret(integer) TO PUBLIC;